...
function TransactionsPage() {
  ...
  const fetchTransactions = async () => {
    try {
      const response = await fetch(
        `${config.backendURL}/transactions?from=2024-09-01T00:00:00.000Z&to=2024-10-01T00:00:00.000Z`
      );
      if (!response.ok) {
  ...
  const fetchCategorySummaries = async () => {
    try {
      const response = await fetch(
        `${config.backendURL}/transactions?from=2024-09-01T00:00:00.000Z&to=2024-10-01T00:00:00.000Z&list_type=group_by_category`
      );
      const data = await response.json();
